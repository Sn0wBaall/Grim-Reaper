...
def help_panel():
    ...
    print(f"{colored('ARP scanner', 'yellow', attrs=['bold'])} {colored('(', 'white', attrs=['bold'])}{colored('EXECUTE IT AS ROOT', 'red', 'on_white', attrs=['reverse'])}{colored(')', 'white', attrs=['bold'])}{colored(':', 'yellow', attrs=['bold'])}")
...
def complete_scan(ip_range, interface, num_threads):
    ...
    p1.success(f"{colored('Done', 'white')}")
